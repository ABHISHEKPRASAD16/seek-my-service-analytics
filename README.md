...
> **Research question:** can model-monitoring signals detect a silent pipeline
> failure before accuracy metrics degrade?
> **Answer:** yes — by four months, using a metric that costs nothing to collect
> and that almost nobody monitors. [Jump to the evidence](#the-drift-incident).
...
**This is a Master's thesis project at IU Internationale Hochschule, Berlin, and
the synthetic data is a methodological requirement rather than a compromise.**

The thesis asks whether model-monitoring signals can detect a silent pipeline
failure *before* accuracy metrics degrade. Testing that claim needs a dataset
where the failure is **injected at a known date**, so detection can be measured
against ground truth. Production data does not come labelled "the retrain job
stopped working here" — which is precisely the problem the thesis is about.

So the failure is placed deliberately: the retrain job stops on **15 March
2026**, accuracy holds until the monsoon shifts the demand regime on **1 June**,
and the question is which signal moves first. Training-data age moves in March.
Accuracy moves in June. That gap is the result.
